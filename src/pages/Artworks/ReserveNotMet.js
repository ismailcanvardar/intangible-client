...
function ReserveNotMet() {
  ...
  const getAuctions = () => {
    axios
      .get(`${process.env.REACT_APP_API_ENDPOINT}/auctions/reservenotmet`)
      .then((res) => {
    ...
    auctions && (
      <div className="py-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {auctions.length > 0 ? (
          ...
          <p className="text-text-primary text-xl">No results found.</p>
        )}
      </div>
    )
  );
